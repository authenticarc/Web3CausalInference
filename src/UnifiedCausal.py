...
def _area_cumgain_centered(psi, scores):
    ...
    psi_ord = psi[order]
    psi_c = psi_ord - psi_ord.mean()    # 中心化，避免 oracle 面积≤0 的不稳
...
class UnifiedCausalTester:
    ...
    def __init__(self,
        ...
        self.regressor = regressor or CatBoostRegressor(
            ...
            subsample=0.9, verbose=0, random_seed=random_state, thread_count=1
        )
    ...
    def _crossfit_predictions_parallel(self, X, T, Y):
        ...
        if self.n_jobs in (None, 1):
            ...
            if _HAS_TQDM_JOBLIB:
                with tqdm_joblib(tqdm(total=len(tasks), disable=(self.verbose==0), desc="[CF|Parallel]")):
                    results = Parallel(n_jobs=self.n_jobs, backend="loky", verbose=0)(tasks)
            else:
                if self.verbose:
                    print("[CF|Parallel] running... (install tqdm_joblib 可显示实时进度)")
                results = Parallel(n_jobs=self.n_jobs, backend="loky", verbose=0)(tasks)
    ...
    def _estimate_core(self, X, T, Y):
        ...
        if self.estimand == "ATE":
            ...
            # 为了 nAUUC 统一，在 ATT 里也把 psi 暴露出去（可按 AIPW 定义）
    ...
    def _diagnostics(self, X, T, diag, X_names=None):
        ...
        X = np.asarray(X)
        if X_names is None: X_names = [f"x{j}" for j in range(X.shape[1])]
        smd = {}
    ...
    # --- nAUUC（OOF，overlap 带内） ---
    def _compute_nauuc_oof(self, X, T, Y, e, ks=None):
        if not self.compute_nauuc:
            return None
        lo, hi = self.nauuc_band
        band = (e >= lo) & (e <= hi)
        if band.sum() < max(50, self.n_splits*3) or (T[band]==1).sum()==0 or (T[band]==0).sum()==0:
            return dict(enabled=True, note="band too small or mono treatment", band=(float(lo),float(hi)),
                        coverage=float(band.mean()), n=int(band.sum()))
        ...
        for tr, va in kf.split(Xb):
            ...
            # 构造 ψ_oof（AIPW）
            clf_cv = clone(self.classifier); reg1 = clone(self.regressor); reg0 = clone(self.regressor)
            for mdl in (clf_cv, reg1, reg0):
                if hasattr(mdl, "set_params"):
                    try: mdl.set_params(thread_count=1)
                    except: pass
            clf_cv.fit(X_tr, T_tr)
            e_va = np.clip(clf_cv.predict_proba(X_va)[:,1], self.trim, 1-self.trim)
            ...
            # τ̂_oof：ForestDRLearner
            if (T_tr==1).any() and (T_tr==0).any():
                dr = ForestDRLearner(
                    model_regression=clone(self.regressor),
                    model_propensity=clone(self.classifier),
                    random_state=self.random_state, categories='auto'
                )
                # 防止内层多线程
                for mdl in (dr.model_regression, dr.model_propensity):
                    if hasattr(mdl, "set_params"):
                        try: mdl.set_params(thread_count=1)
                        except: pass
        ...
        if len(psi_oof) < max(30, self.n_splits*2):
            return dict(enabled=True, note="too few valid oof points", band=(float(lo),float(hi)),
                        coverage=float(band.mean()), n=int(band.sum()))
        ...
        return dict(
            enabled=True, band=(float(lo),float(hi)),
            coverage=float(band.mean()), n=int(band.sum()),
            nauuc=nauuc, area_model=area_model, area_oracle=area_oracle, **pol
        )

    # --- 拟合（placebo/负控并行 + tqdm） ---
    def fit(self, X, T, Y, X_names=None, y_nc=None, placebo_runs=300):
        # 主估计 & 诊断
        est, ci, diag = self._estimate_core(X, T, Y)
        diag2 = self._diagnostics(X, T, diag, X_names=X_names)

        # === nAUUC（基于 OOF ψ 与 OOF τ̂，overlap 带内）===
        auuc = self._compute_nauuc_oof(np.asarray(X), np.asarray(T).astype(int), np.asarray(Y).astype(float), diag["e"])
        ...
        if self.n_jobs_placebo in (None, 1):
            ...
            for s in tqdm(seeds, total=placebo_runs, disable=(self.verbose==0), desc="[Placebo]"):
                ...
                v, _, _ = self._estimate_core(X, T_fake, Y)
            ...
            def _one_placebo(seed):
                ...
                return v
            if _HAS_TQDM_JOBLIB:
                with tqdm_joblib(tqdm(total=placebo_runs, disable=(self.verbose==0), desc="[Placebo|Parallel]")):
                    placebo_vals = Parallel(n_jobs=self.n_jobs_placebo, backend="loky")(delayed(_one_placebo)(int(s)) for s in seeds)
            else:
                if self.verbose:
                    print("[Placebo|Parallel] running... (install tqdm_joblib 可显示实时进度)")
                placebo_vals = Parallel(n_jobs=self.n_jobs_placebo, backend="loky")(delayed(_one_placebo)(int(s)) for s in seeds)
        ...
        if y_nc is not None:
            ...
            if self.n_jobs_placebo in (None, 1):
                ...
                for s in tqdm(seeds, total=placebo_runs, disable=(self.verbose==0), desc="[NegCtrl]"):
                    rng_local = np.random.default_rng(int(s))
                    T_fake = rng_local.permutation(T)
                    v, _, _ = self._estimate_core(X, T_fake, y_nc)
                    vals.append(v)
            else:
                def _one_nc(seed):
                    rng_local = np.random.default_rng(int(seed))
                    ...
                    return v
                if _HAS_TQDM_JOBLIB:
                    with tqdm_joblib(tqdm(total=placebo_runs, disable=(self.verbose==0), desc="[NegCtrl|Parallel]")):
                        vals = Parallel(n_jobs=self.n_jobs_placebo, backend="loky")(delayed(_one_nc)(int(s)) for s in seeds)
                else:
                    if self.verbose:
                        print("[NegCtrl|Parallel] running... (install tqdm_joblib 可显示实时进度)")
                    vals = Parallel(n_jobs=self.n_jobs_placebo, backend="loky")(delayed(_one_nc)(int(s)) for s in seeds)
            est_nc = float(self._estimate_core(X, T, y_nc)[0])
    ...
    def report(self):
        ...
        R = self.result_
        lines = []
        lines.append(f"【{R['estimand']} 可信度诊断报告（并行 + tqdm）】")
        ...
        if isinstance(R.get("auuc"), dict) and R["auuc"] is not None:
            ...
            if A.get("enabled"):
                lines.append(
                    f"- ForestDR 排序能力（overlap e∈[{self.nauuc_band[0]:.2f},{self.nauuc_band[1]:.2f}]，OOF）"
