...
class NAUUCCatBoostTuner:
    ...
    def _objective(self, trial, X, T, Y):
        reg_params = {
            "depth": trial.suggest_int("reg_depth", 4, 10),
        ...
        clf_params = {
            ...
            "auto_class_weights": trial.suggest_categorical("clf_class_wt", ["Balanced","SqrtBalanced"]),
            "random_seed": self.random_seed,
            "verbose": 0
        }
    ...
    def fit_return_models(self, X, T, Y) -> Tuple[CatBoostRegressor, CatBoostClassifier]:
        ...
        self.best_clf_ = CatBoostClassifier(
            ...
            auto_class_weights=self.best_params_["clf_class_wt"],
            random_seed=self.random_seed,
            verbose=0
        )
